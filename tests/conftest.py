...
def sandbox_state(tmp_path, monkeypatch):
    ...
    escaped = _snapshot(list(roots)) - before
    assert not escaped, (
        "test wrote into the real project tree:\n  "
        + "\n  ".join(sorted(str(p) for p in escaped))
    )
